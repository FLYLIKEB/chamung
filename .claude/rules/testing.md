...
- Cleanup 순서: `email_change_tokens` → `email_verification_tokens` → `note_bookmarks` → `note_likes` → `note_axis_value` → `note_tags` → `tags` → `notes` → `cellar_items` → `rating_axis` → `rating_schema` → `audit_logs` → `teas` → `sellers` → `user_authentications` → `users`
