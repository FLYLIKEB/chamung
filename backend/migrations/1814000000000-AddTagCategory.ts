...
export class AddTagCategory1814000000000 implements MigrationInterface {
    ...
    private async addForeignKeyIfNotExists(queryRunner: QueryRunner, tableName: string, fkName: string, fkDef: string): Promise<void> {
        this.validateIdentifier(tableName);
        ...
        if (Number(result[0].cnt) === 0) {
            await queryRunner.query(`ALTER TABLE \`${tableName}\` ADD CONSTRAINT \`${fkName}\` ${fkDef}`);
        }
    }
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await this.createIndexIfNotExists(queryRunner, 'password_resets', 'IDX_7f6aae0fcc807c9e7194ca5cc4', '`tokenHash`', true);
        ...
        // refresh_tokens.tokenHash: drop old varchar(64) and recreate as varchar(255)
        if (await this.columnExists(queryRunner, 'refresh_tokens', 'tokenHash')) {
            const colInfo = await queryRunner.query(
                `SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'refresh_tokens' AND COLUMN_NAME = 'tokenHash'`,
            );
            if (colInfo.length > 0 && colInfo[0].COLUMN_TYPE === 'varchar(64)') {
                await this.dropIndexIfExists(queryRunner, 'refresh_tokens', 'UQ_c25bc63d248ca90e8dcc1d92d06');
        ...
        await this.createIndexIfNotExists(queryRunner, 'tea_wishlists', 'IDX_3f82647610d8c8964b4edbda60', '`teaId`');
        await this.createIndexIfNotExists(queryRunner, 'tea_wishlists', 'IDX_f6d2e7bbe89d4d2bb8d9aeaba9', '`teaId`, `userId`', true);
        await this.createIndexIfNotExists(queryRunner, 'post_images', 'IDX_92e2382a7f43d4e9350d591fb6', '`postId`');
        await this.addForeignKeyIfNotExists(queryRunner, 'password_resets', 'FK_d95569f623f28a0bf034a55099e', `FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'note_schemas', 'FK_2cb2d20d688b9bb828dd6ff566b', `FOREIGN KEY (\`noteId\`) REFERENCES \`notes\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'note_schemas', 'FK_99116e60a8543f91cbde0aad562', `FOREIGN KEY (\`schemaId\`) REFERENCES \`rating_schema\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'teas', 'FK_1c485b6b74287fbc93221ff305c', `FOREIGN KEY (\`sellerId\`) REFERENCES \`sellers\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'tea_wishlists', 'FK_3f82647610d8c8964b4edbda603', `FOREIGN KEY (\`teaId\`) REFERENCES \`teas\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'tea_wishlists', 'FK_485ebd115d7891009ef74142ca9', `FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'tea_session_steeps', 'FK_faf4e470429cd6e70cce197e253', `FOREIGN KEY (\`sessionId\`) REFERENCES \`tea_sessions\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'tea_sessions', 'FK_8f928a87387a4332e80ba51ce56', `FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'tea_sessions', 'FK_e2fd96d3530a0950e35c7d06b33', `FOREIGN KEY (\`teaId\`) REFERENCES \`teas\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'tea_sessions', 'FK_c9ffb463a670be3b398b9d9321e', `FOREIGN KEY (\`noteId\`) REFERENCES \`notes\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'post_images', 'FK_92e2382a7f43d4e9350d591fb6a', `FOREIGN KEY (\`postId\`) REFERENCES \`posts\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_session_participants', 'FK_eb77aa981951b86b443635e90aa', `FOREIGN KEY (\`sessionId\`) REFERENCES \`blind_tasting_sessions\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_session_participants', 'FK_cc08133acd21548c252d85c1878', `FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_session_participants', 'FK_62f4b02ff699eb737a218e05c36', `FOREIGN KEY (\`noteId\`) REFERENCES \`notes\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_session_participant_notes', 'FK_41a1614b2edb16501fb3da0285f', `FOREIGN KEY (\`participantId\`) REFERENCES \`blind_session_participants\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_session_participant_notes', 'FK_0cca861dfa108152f135df33dd9', `FOREIGN KEY (\`roundId\`) REFERENCES \`blind_session_rounds\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_session_participant_notes', 'FK_0731579ea21abf3f3467ad67ee5', `FOREIGN KEY (\`noteId\`) REFERENCES \`notes\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_session_rounds', 'FK_2834aac794799727b55507845a8', `FOREIGN KEY (\`sessionId\`) REFERENCES \`blind_tasting_sessions\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_session_rounds', 'FK_c0a5f55d3b0af62ea512f110329', `FOREIGN KEY (\`teaId\`) REFERENCES \`teas\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_tasting_sessions', 'FK_bb54a646e566b4fc78a0336d15d', `FOREIGN KEY (\`hostId\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await this.addForeignKeyIfNotExists(queryRunner, 'blind_tasting_sessions', 'FK_bf029e3e665d1b3c2866de83a25', `FOREIGN KEY (\`teaId\`) REFERENCES \`teas\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION`);
    }
